...
def healPlanReplyParser(data, c):
	...
	if t[2] == 'accepted':
		...
		ClientCreator(
			reactor, PlanSender, **monitoringArguments
		).connectTCP(
			argv[1], int(argv[2]))
...
# [1] monitoring group's peer's address
# [2] monitoring group's peer's port
# [3] monitoring plan

# [4] healing group's peer's address
# [5] healing group's peer's port
# [6] healing plan

ClientCreator(
	reactor, PlanSender, **healArguments
).connectTCP(
	argv[4], int(argv[5]))
